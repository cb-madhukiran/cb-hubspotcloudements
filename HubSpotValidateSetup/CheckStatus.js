...
if(steps.GetExecutionStatus.data.status === undefined) {
  input.error = true;
  input.done = true;
}else if(steps.GetExecutionStatus.data.status === "error"){
  input.error = true;
  input.done = true;
}else if(steps.GetExecutionStatus.data.status === "success"){
  input.error = false;
  input.done = true;
  
  input.result = steps.GetExecutionStatus.data.validation;
...
}else if(steps.GetExecutionStatus.data.status === "pending"){
  input.error = false;
  input.done = false;
}
